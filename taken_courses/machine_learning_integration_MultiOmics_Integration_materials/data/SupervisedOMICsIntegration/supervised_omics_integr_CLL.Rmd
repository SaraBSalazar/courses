...
for(i in 1:ncol(expr)){expr[,i][is.na(expr[,i])]<-median(expr[,i],na.rm=TRUE)} #filling the gaps in the data by the median
expr[1:5,1:5]
...
```{r,fig.width=10,fig.height=8}
set.seed(1234)
female_sample<-rownames(expr)[as.character(phen$Gender)=="f"][sample(1:length(rownames(expr)[as.character(phen$Gender)=="f"]),24)] # pick random but 24
female_sample
...
#reduction of the dimensionallity
#remove mutations 0 everywhere, adds no variability
...
Right now we can not integrate the OMICs data sets since at least expression and methylation data are high-dimensional so we need to perform a feature selection for those two OMICs. Here we use LASSO for doing feature selection for gene expression data: (drug didn't had to many features)
...
#This matrix defines how they are linked.  In this case it wasn't decided there is a correlation, no assumptions.
#MOFA does pairwise correlation
design = matrix(1, ncol = length(data), nrow = length(data), dimnames = list(names(data), names(data)))
diag(design) = 0
design

#Cross validation, how many pls components are informative? To avoid overfitting
splsda.res = block.splsda(X = data, Y = Y, ncomp = 8, design = design)

# Set up parallel backend (4 workers)
BPPARAM <- SnowParam(workers = 4)

perf.diablo = perf(splsda.res, validation = 'Mfold', folds = 2, nrepeat = 5,
                    progressBar = FALSE, BPPARAM = BPPARAM)   # <- use the object, not 4
...
ptm<-proc.time()
tune.omics=tune.block.splsda(X=data,Y=Y,ncomp=2,test.keepX=test.keepX,design=design,progressBar=TRUE,validation="Mfold",folds=2,nrepeat=5,near.zero.var=FALSE,dist = "mahalanobis.dist") # ncomp was defined prior
...
```{r,fig.width=10,fig.height=8}
tune.omics$choice.keepX # features choosen for each of the OMICs
```
...
selectVar(res,block='expr',comp=1)$expr$name # see wich var were selected
...
table(predict.diablo$MajorityVote$mahalanobis.dist[,1],Y.test)
round((sum(diag(table(predict.diablo$MajorityVote$mahalanobis.dist[,1],Y.test)))/sum(table(predict.diablo$MajorityVote$mahalanobis.dist[,1],Y.test)))*100) # only 72% probablility
